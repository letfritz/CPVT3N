...
% ARQUIVO: Código iterativo das variáveis do reservatorio
...
function [RESERVATORIO,Ts,Q_modulo_real,T_placa,T_cell,efic_cell,...
    ...
    RESERVATORIO.T = zeros(maxPontos,4);
    RESERVATORIO.T(1,1) = sum(RESERVATORIO.Tagua)/3;
    RESERVATORIO.T(1,2:3) = RESERVATORIO.T(1);
    RESERVATORIO.T(1,4) = CURVA.vTemperatura(1,1)+273+2;
    
    % Energia Elétrica da Resistência que esquenta a água do nível 1
    RESERVATORIO.Q_eleVetor = zeros(maxPontos,1);
    RESERVATORIO.Q_ele = 0;
    ...
    for minuto = 1:maxPontos
        ...
        if CURVA.vDNI(minuto,1)<0.1
            T_cell(minuto,1) = Ta+...
                (Voc-Voc_0)/abs(-0.006424+0.00036233*log(COLETOR.CF)*100);
        else
            ...
            T_cell_aux = [COLETOR.emiss*GERAL.constSB*Ap,0,0,...
                (COLETOR.coef_ConvMedio*Ap+(6*kcob*Ac/COLETOR.Lplaca)),...
        ...
        efic_cell(minuto,1) = COLETOR.efic_ref+...
            ((-0.09167+0.005787*log(COLETOR.CF))/100)*...
            (T_cell(minuto,1)-273-25);
        
        % Coeficiente de potência térmico
        coef_termico(minuto,1) = 1+COLETOR.coef_temperatura*...
            (T_cell(minuto,1)-273-25);
        
        % Eficiência do módulo concentrador fotovoltaico
        efic_PV = efic_cell(minuto,1)*COLETOR.efic_modulo*...
            coef_termico(minuto,1);
        ...
        RESERVATORIO.T(minuto+1,1) = sum(RESERVATORIO.Tagua(minuto,:))/3;
        ...
        % Potência térmica liberada pelo módulo considerando as perdas [kW]
        Q_modulo_real(minuto,1) = (Q_modulo-Q_perdido+COLETOR.Q_gerado);
        
        %% CÁLCULO DA NOVA TEMPERATURA DA PLACA
        
        % Temperatura da placa [K]
        T_placa(minuto+1,1) = T_cell(minuto,1)-Q_modulo_real(minuto,1)/...
        ...
        if CURVA.vDNI(minuto,1)>0.1
            ...
            if relacaoDNI<0.895 % Mínimo é 88 ºC
                relacaoDNI = 0.895;
            ...
            mass_flow(minuto+1,1) = Q_modulo_real(minuto,1)/...
                (cpH2O*(Ts(minuto+1,1)-RESERVATORIO.Tagua(minuto+1,3)));
            ...
            % Número de Reynolds
            Reynolds = (4*mass_flow(minuto+1,1))/(uH2O*pi*RESERVATORIO.d);
            ...
            T_placa(minuto+1,1) = (RESERVATORIO.Tagua(minuto+1,3)-...
                exp((h_agua*Ac)/((mass_flow(minuto+1,1)/6)*(cpH2O)))*...
                Ts(minuto+1,1))/...
                (1-exp((h_agua*Ac)/((mass_flow(minuto+1,1)/6)*(cpH2O))));
    ...
    %% Energia total gasta pela resistência de aquecimento do primeiro nível
    RESERVATORIO.Q_ele = sum(RESERVATORIO.Q_eleVetor);
